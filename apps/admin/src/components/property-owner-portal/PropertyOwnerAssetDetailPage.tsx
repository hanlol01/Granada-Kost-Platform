    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-5 p-6 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="secondary">{label(asset.kostType)}</Badge>
              <Badge variant="secondary">{label(asset.genderPolicy)}</Badge>
            </div>
          ...
          <div className="rounded-xl border border-primary/20 bg-primary/[0.045] px-4 py-3 text-sm text-muted-foreground">
            ...
            <p className="mt-1">Data mengikuti assignment kepemilikan yang berlaku.</p>
          </div>
        </div>
      </section>
      <section className="grid items-stretch gap-4 xl:grid-cols-3">
        <Card className="border-border/80 shadow-sm">
          ...
          <CardContent className="grid gap-5 pt-6 sm:grid-cols-2 xl:grid-cols-1">
            <DataItem label="Nomor kamar" value={asset.roomCode} />
            <DataItem label="Kode kamar" value={asset.roomCode} />
            <DataItem label="Bangunan" value={asset.building.name} />
            <DataItem label="Kategori" value={label(asset.kostType)} />
            <DataItem
              label="Unit"
        ...
        <Card className="border-border/80 shadow-sm">
          <CardHeader className="border-b border-border/70 pb-4">
            <CardTitle className="flex items-center gap-2 text-base">
              <CircleDollarSign className="h-4 w-4 text-primary" /> Sumber komersial kategori
            </CardTitle>
          </CardHeader>
          <CardContent className="grid gap-5 pt-6 sm:grid-cols-2 xl:grid-cols-1">
            <DataItem
        ...
        <Card className="border-border/80 shadow-sm">
          ...
          <CardContent className="grid gap-5 pt-6">
            <DataItem
              label="Otoritas saat ini"
              value={
                asset.ownership.source === "building_assignment"
                  ? asset.building.name
                  : "Owner Property"
              }
            />
      ...
      <section className="grid items-stretch gap-4 lg:grid-cols-2">
        <Card className="border-border/80 shadow-sm">
          <CardHeader className="border-b border-border/70 pb-4">
            <CardTitle className="flex items-center gap-2 text-base">
              <UserRound className="h-4 w-4 text-primary" /> Penghuni aktif
            </CardTitle>
          </CardHeader>
          <CardContent className="grid gap-5 pt-6 sm:grid-cols-2">
            <DataItem
              label="Nama penghuni"
              value={resident?.displayName ?? "Tidak ada penghuni aktif"}
            />
            <DataItem label="Status akun" value={resident ? "Aktif" : "—"} />
            <DataItem
              label="Mulai hunian"
              value={resident ? date(resident.occupancyStartDate) : "Tidak tercatat"}
            />
            <DataItem
              label="Status hunian"
              value={
                residentDetail?.occupancy ? label(residentDetail.occupancy.status) : "Tidak aktif"
              }
            />
            {resident ? (
              <Button asChild className="min-h-10 sm:col-span-2">
                <Link
                  to="/property-owners/portal/occupancy/$roomCode"
                  params={{ roomCode: asset.roomCode }}
                >
                  <UserRound className="mr-2 h-4 w-4" /> Buka detail penghuni
                </Link>
              </Button>
            ) : null}
          </CardContent>
        ...
        <Card className="border-border/80 shadow-sm">
          <CardHeader className="border-b border-border/70 pb-4">
            <CardTitle className="flex items-center gap-2 text-base">
              <ClipboardList className="h-4 w-4 text-primary" /> Penyewaan dan hunian
            </CardTitle>
          </CardHeader>
          <CardContent className="grid gap-5 pt-6 sm:grid-cols-2">
            <DataItem
              label="Kode penyewaan"
              value={lease ? "Tersedia pada data sewa" : "Tidak tercatat"}
            />
            <DataItem
      ...
      <Card className="border-border/80 shadow-sm">
        <CardHeader className="border-b border-border/70 pb-4">
          <CardTitle className="flex items-center gap-2 text-base">
            <CircleDollarSign className="h-4 w-4 text-primary" /> Ringkasan penyewaan dan pembayaran
          </CardTitle>
        </CardHeader>
          ...
          {collectionItem ? (
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <Metric
                ...
                value={`${collectionItem.billing.overdueCount} invoice`}
              />
              <Metric label="Tagihan berikutnya" value={date(collectionItem.billing.nextDueDate)} />
              <Metric
                label="Angsuran"
              ...
              <Metric
                ...
                value={
                  collectionItem.settlement.checkpoint.status === "not_available"
                    ? "Belum tersedia"
                    : label(collectionItem.settlement.checkpoint.status)
                }
              />
              <Metric
                label="Minimum checkpoint"
                value={formatOwnerMoney(collectionItem.settlement.checkpoint.requiredAmount)}
              />
              <Metric
                label="Kredit checkpoint"
                value={formatOwnerMoney(collectionItem.settlement.checkpoint.receivedAmount)}
              />
              <Metric
                label="Kekurangan checkpoint"
                value={formatOwnerMoney(collectionItem.settlement.checkpoint.remainingAmount)}
              />
              <Metric
                label="Tenggat checkpoint"
                value={date(collectionItem.settlement.checkpoint.dueAt)}
              />
              <Metric
                label="Pengingat tagihan"
                value={collectionItem.settlement.reminderStage ?? "Tidak ada"}
              />
            </div>
          ) : billing ? (
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <p className="rounded-xl border border-border/70 bg-muted/25 p-4 text-sm text-muted-foreground">
              Data billing belum tersedia untuk penghuni aktif.
            </p>
          )}
        </CardContent>
      </Card>
      <section className="grid gap-4 lg:grid-cols-2">
        <Card className="border-border/80 shadow-sm">
          <CardHeader className="border-b border-border/70 pb-4">
            <CardTitle className="flex items-center gap-2 text-base">
              <CarFront className="h-4 w-4 text-primary" /> Kendaraan dan parkir
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">
              Ringkasan kendaraan aktif dan slot parkir dalam cakupan kamar ini.
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <Metric label="Kendaraan aktif" value={`${operations?.activeVehicleCount ?? 0}`} />
              <Metric
                label="Slot parkir terhubung"
                value={`${operations?.assignedParkingCount ?? 0}`}
              />
            </div>
          </CardContent>
        </Card>
        <Card className="border-border/80 shadow-sm">
          <CardHeader className="border-b border-border/70 pb-4">
            <CardTitle className="flex items-center gap-2 text-base">
              <MessageSquareWarning className="h-4 w-4 text-primary" /> Komplain dan work order
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
            <div className="grid gap-3 sm:grid-cols-2">
              <Metric label="Komplain terbuka" value={`${asset.issues.openComplaints}`} />
              <Metric label="Maintenance aktif" value={`${asset.issues.openMaintenance}`} />
            </div>
            <p className="mt-4 text-sm text-muted-foreground">
              Rincian pekerjaan operasional dikelola melalui Admin.
            </p>
          </CardContent>
        </Card>
      </section>
      <Card className="border-border/80 shadow-sm">
        <CardHeader className="border-b border-border/70 pb-4">
          <CardTitle className="flex items-center gap-2 text-base">
            <History className="h-4 w-4 text-primary" /> Aktivitas kamar
          </CardTitle>
        </CardHeader>
        <CardContent className="pt-6">
          {activity.length ? (
            ...
            <p className="text-sm text-muted-foreground">
              Belum ada aktivitas ringkas untuk kamar ini.
            </p>
          )}
        </CardContent>
      </Card>
...
export function PropertyOwnerAssetDetailPage({ roomCode }: { roomCode: string }) {
  ...
  const residentDetail = useQuery({
    ...
    enabled: Boolean(user?.id && hasRole("property_owner") && detail.data?.resident),
  });
  const collection = useQuery({
    queryKey: ["property-owner", "collection-progress", user?.id],
      ...
      {detail.isLoading ? (
        ...
        <>
          <div className="mb-5 flex items-center gap-2 text-sm text-muted-foreground">
            <CalendarClock className="h-4 w-4 text-primary" /> Diperbarui{" "}
            {date(detail.data.updatedAt)}
          </div>
          <DetailContent
            asset={detail.data}
            residentDetail={residentDetail.data}
