...
function AccessHistoryPage() {
  ...
  const filtered = useMemo(() => {
    return accessLogs.filter((l) => {
      ...
      if (query && !`${l.tenantName} ${l.roomNumber}`.toLowerCase().includes(query.toLowerCase()))
        return false;
      return true;
    });
    ...
    <AppShell
      ...
      actions={
        <Button
          size="sm"
          variant="outline"
          onClick={() => toast.success("Export CSV berhasil (dummy)")}
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">Total Akses</p>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-semibold">{failCount}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
                ...
                {paged.map((l) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-3">
                      <span
                        className={`inline-flex items-center gap-1.5 text-xs font-medium ${l.type === "lock" ? "text-primary" : "text-success"}`}
                      >
                        {l.type === "lock" ? (
              ...
              <div key={l.id} className="rounded-lg border border-border p-3 animate-fade-in">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div
                      className={`h-8 w-8 rounded-lg flex items-center justify-center ${l.type === "lock" ? "bg-primary-soft text-primary" : "bg-success/15 text-success"}`}
                    >
                      {l.type === "lock" ? (
