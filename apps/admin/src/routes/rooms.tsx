...
function RoomsPage() {
  ...
  const filtered = useMemo(
    () => rooms.filter((r) => (filter === "all" || r.status === filter) && (q === "" || r.number.includes(q) || r.type.toLowerCase().includes(q.toLowerCase()))),
    [rooms, q, filter],
  );
    ...
    >
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cari nomor atau tipe kamar..." className="pl-9" />
        </div>
          ...
          {filtered.map((r) => (
            <Card key={r.id} className="hover:shadow-md transition-all hover:-translate-y-0.5">
              <CardContent className="p-5">
                ...
                <p className="text-lg font-semibold text-primary">{formatIDR(r.price)}<span className="text-xs text-muted-foreground font-normal">/bulan</span></p>
                <div className="flex flex-wrap gap-1 mt-3 mb-4 min-h-[2rem]">
                  ...
                  {r.facilities.length > 4 && <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground">+{r.facilities.length - 4}</span>}
                </div>
                <div className="flex gap-2 pt-3 border-t border-border">
                  <Button variant="outline" size="sm" className="flex-1" onClick={() => { setEditing(r); setOpen(true); }}>
      ...
      <RoomDialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setEditing(null); }} room={editing} onSave={onSave} />
    </AppShell>
  );
}

function RoomDialog({ open, onOpenChange, room, onSave }: { open: boolean; onOpenChange: (o: boolean) => void; room: Room | null; onSave: (r: Room) => void }) {
  return (
        ...
        <form
          onSubmit={(e) => {
            ...
            onSave({
              ...
              facilities: (f.get("facilities") as string).split(",").map((s) => s.trim()).filter(Boolean),
            });
          }}
          className="space-y-4"
        >
          <div className="grid grid-cols-2 gap-3">
            <div><Label htmlFor="number">Nomor</Label><Input id="number" name="number" defaultValue={room?.number} required /></div>
            <div><Label htmlFor="type">Tipe</Label><Input id="type" name="type" defaultValue={room?.type ?? "Standard"} required /></div>
          </div>
          <div><Label htmlFor="price">Harga / bulan</Label><Input id="price" name="price" type="number" defaultValue={room?.price ?? 800000} required /></div>
          <div>
          ...
          <div><Label htmlFor="facilities">Fasilitas (pisah dengan koma)</Label><Input id="facilities" name="facilities" defaultValue={room?.facilities.join(", ") ?? "Kasur, Lemari"} /></div>
          <DialogFooter>
