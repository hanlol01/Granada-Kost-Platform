...
function SettingsPage() {
  ...
  return (
    <AppShell title="Pengaturan" subtitle="Konfigurasi rumah kos dan preferensi">
      <form
        onSubmit={(e) => { e.preventDefault(); toast.success("Pengaturan disimpan"); }}
        className="grid grid-cols-1 lg:grid-cols-3 gap-4"
      >
        <Card className="lg:col-span-2">
          <CardHeader><CardTitle className="text-base">Informasi Rumah Kos</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div><Label htmlFor="name">Nama Rumah Kos</Label><Input id="name" defaultValue="Kos Mawar Indah" /></div>
            <div><Label htmlFor="addr">Alamat</Label><Textarea id="addr" defaultValue="Jl. Mawar No. 12, Kelurahan Sukamaju, Jakarta Selatan" /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label htmlFor="phone">Nomor Kontak</Label><Input id="phone" defaultValue="081234567890" /></div>
              <div><Label htmlFor="email">Email</Label><Input id="email" type="email" defaultValue="admin@kosmawar.id" /></div>
            </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="border-2 border-dashed border-border rounded-xl p-6 text-center hover:bg-muted/30 transition-colors cursor-pointer">
                ...
                <Button type="button" variant="outline" size="sm" className="mt-3" onClick={(e) => { e.preventDefault(); toast.info("Fitur dummy"); }}>
