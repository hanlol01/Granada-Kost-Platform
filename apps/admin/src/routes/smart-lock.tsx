...
function LockCard({
  ...
  lock: SmartLock;
  onAction: (l: SmartLock, action: "lock" | "unlock") => void;
  onOpen: (l: SmartLock) => void;
}) {
    ...
    <div className="group relative rounded-xl border border-border bg-card/80 backdrop-blur p-5 hover:shadow-lg hover:-translate-y-0.5 transition-all">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div
            className={`h-11 w-11 rounded-xl flex items-center justify-center shrink-0 ${locked ? "bg-primary text-primary-foreground" : "bg-success/15 text-success"}`}
          >
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground truncate">
              {lock.tenantName ?? "Kosong"} · {lock.deviceId}
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="rounded-lg bg-muted/50 p-3">
          ...
          <p className="text-sm font-semibold mt-1 flex items-center gap-1.5">
            {restricted ? (
              <>
                <Shield className="h-3.5 w-3.5 text-destructive" /> Restricted
              </>
            ) : locked ? (
              <>
                <DoorClosed className="h-3.5 w-3.5" /> Locked
              </>
            ) : (
              <>
                <DoorOpen className="h-3.5 w-3.5 text-success" /> Unlocked
              </>
            )}
          </p>
        </div>
        <div className="rounded-lg bg-muted/50 p-3">
          <p className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold flex items-center gap-1">
            {lock.battery < 20 ? (
              <BatteryLow className="h-3 w-3 text-destructive" />
            ) : (
      ...
      <div className="mt-4 flex items-center justify-between gap-2">
        <p className="text-[11px] text-muted-foreground flex items-center gap-1">
          <Activity className="h-3 w-3" /> {lock.lastActivity}
        </p>
        <div className="flex items-center gap-1.5">
          <Button
            size="sm"
            variant="outline"
            disabled={lock.connection === "offline" || restricted}
            onClick={() => onAction(lock, locked ? "unlock" : "lock")}
          >
            {locked ? <Unlock className="h-3.5 w-3.5" /> : <Lock className="h-3.5 w-3.5" />}
            <span className="ml-1">{locked ? "Unlock" : "Lock"}</span>
          </Button>
          <Button size="sm" variant="ghost" onClick={() => onOpen(lock)}>
            Detail
          </Button>
        </div>
      </div>
...
function SmartLockPage() {
  ...
  const filtered = list.filter((l) =>
    [l.roomNumber, l.tenantName ?? "", l.deviceId]
      .join(" ")
      .toLowerCase()
  ...
  const confirmAction = () => {
    ...
    setTimeout(() => {
      setList((prev) =>
        prev.map((l) =>
          l.id === lock.id
            ? { ...l, state: action === "lock" ? "locked" : "unlocked", lastActivity: "Baru saja" }
            : l,
        ),
      );
      setBusyId(null);
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <Card className="lg:col-span-2">
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2">
              <Activity className="h-4 w-4 text-primary" /> Aktivitas Lock / Unlock Hari Ini
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-56">
                ...
                <AreaChart data={lockActivityHourly}>
                  <defs>
                    <linearGradient id="lockG" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="oklch(0.55 0.18 264)" stopOpacity={0.5} />
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2">
              <AlertTriangle className="h-4 w-4 text-warning" /> Alert Smart Lock
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-2.5">
            {lockAlerts.map((a) => (
              ...
              >
                <div
                  className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 ${a.severity === "danger" ? "bg-destructive/15 text-destructive" : a.severity === "warning" ? "bg-warning/15 text-warning" : "bg-primary-soft text-primary"}`}
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map((l) => (
              <div key={l.id} className={busyId === l.id ? "opacity-70 animate-pulse" : ""}>
                <LockCard
                  lock={l}
                  onAction={(lock, action) => setPending({ lock, action })}
                  onOpen={setDetail}
                />
              </div>
            ))}
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Shield className="h-5 w-5 text-primary" /> Konfirmasi{" "}
              {pending?.action === "lock" ? "Lock" : "Unlock"}
            </DialogTitle>
            <DialogDescription>
              Perintah {pending?.action} akan dikirim ke device{" "}
              <span className="font-medium text-foreground">{pending?.lock.deviceId}</span> kamar{" "}
              {pending?.lock.roomNumber}.
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent>
          {detail && (
            <>
              <DialogHeader>
                <DialogTitle>
                  Kamar {detail.roomNumber} · {detail.deviceId}
                </DialogTitle>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-lg bg-muted/40 p-3">
                  ...
                  <p className="font-medium mt-1 capitalize">{detail.connection}</p>
                </div>
                <div className="rounded-lg bg-muted/40 p-3">
                  <p className="text-xs text-muted-foreground">Status</p>
                  <p className="font-medium mt-1 capitalize">{detail.state}</p>
                </div>
                <div className="rounded-lg bg-muted/40 p-3">
                  <p className="text-xs text-muted-foreground">Battery</p>
                  <p className="font-medium mt-1">{detail.battery}%</p>
                </div>
                <div className="rounded-lg bg-muted/40 p-3">
