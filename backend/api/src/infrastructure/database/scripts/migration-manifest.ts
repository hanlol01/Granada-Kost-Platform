...
export const MIGRATION_MANIFEST: readonly MigrationManifestEntry[] = [
  ...
  },
  {
    ...
    sentinels: [
      ...
      "EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='room_owner_assignments' AND column_name='reason' AND is_nullable='YES')",
      "EXISTS (SELECT 1 FROM pg_constraint WHERE conname='building_owner_assignments_reason_check' AND conrelid=to_regclass('public.building_owner_assignments') AND pg_get_constraintdef(oid) ILIKE '%reason IS NULL%')",
      "EXISTS (SELECT 1 FROM pg_constraint WHERE conname='room_owner_assignments_reason_check' AND conrelid=to_regclass('public.room_owner_assignments') AND pg_get_constraintdef(oid) ILIKE '%reason IS NULL%')",
    ],
  },
  {
    ...
    sentinels: [
      ...
      "EXISTS (SELECT 1 FROM pg_constraint WHERE conname='room_owner_assignments_dates_check' AND conrelid=to_regclass('public.room_owner_assignments') AND pg_get_constraintdef(oid) ILIKE '%effective_until >= effective_from%')",
    ],
  },
  {
    ...
    sentinels: [
      ...
      "pg_get_functiondef('reconcile_property_owner_service_coverage()'::regprocedure) ILIKE '%attributed.gross_amount > allocated%'",
      "pg_get_functiondef('validate_property_owner_earning_authority()'::regprocedure) ILIKE '%payment_purpose NOT IN (''rent'', ''dp'')%'",
    ],
  },
  {
