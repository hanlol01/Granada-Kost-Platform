...
export class LeaseCheckInService {
  ...
  async confirm(
    ...
    return this.database.transaction(async (client) => {
      ...
      const response = await this.confirmLocked(client, actor, leaseId, {
        propertyId: dto.property_id,
        checkedInAt: dto.checked_in_at ?? null,
        notes: dto.notes,
        attemptKey: `lease-check-in:${leaseId}:${idempotencyKey}`,
        context,
      });
      await client.query(
  ...
  /** Shared transactional authority for standalone and combined activation/check-in commands. */
  async confirmLocked(
    client: PoolClient,
    actor: UserAccessContext,
    leaseId: string,
    input: LockedCheckInInput,
    ...
    const conflicts = await client.query<{ occupancy_count: string; lease_count: string }>(
      ...
      [input.propertyId, row.resident_id, row.room_id, leaseId],
    );
    if (
    ...
    const occupancy = await client.query<{ id: string }>(
      ...
      [input.propertyId, row.room_id, row.resident_id, checkedInAt, actor.id],
    );
    ...
    await client.query(
      ...
      [
        ...
        actor.id,
        JSON.stringify({
          source: 'lease_physical_check_in',
          lease_id: leaseId,
    ...
    const leaseUpdated = await client.query(
      ...
      [leaseId, occupancyId, input.propertyId],
    );
    ...
    await client.query(
      ...
      [row.resident_id, input.propertyId],
    );
