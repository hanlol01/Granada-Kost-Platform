...
export class VehicleController {
  ...
  async history(@CurrentUser() user: UserAccessContext, @Param('vehicleId') vehicleId: string) {
    const vehicle = await this.vehicles.get(vehicleId);
    await this.properties.assertCanReadProperty(user, vehicle.propertyId);
    return this.vehicles.listHistory(vehicleId);
  }

  @Get(':vehicleId/files')
  async files(@CurrentUser() user: UserAccessContext, @Param('vehicleId') vehicleId: string) {
    const vehicle = await this.vehicles.get(vehicleId);
    await this.properties.assertCanReadProperty(user, vehicle.propertyId);
    return this.vehicles.listFiles(vehicleId);
  }
  ...
  async create(
    ...
    if (resident.propertyId !== dto.property_id) {
      throw new BadRequestException({
        code: 'RESIDENT_PROPERTY_MISMATCH',
        message: 'Resident is not in selected property',
      });
    }
