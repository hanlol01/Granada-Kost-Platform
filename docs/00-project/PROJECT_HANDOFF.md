...
- M19 (A-E) selesai (2026-07-08): Hunian Gallery - Admin Gallery Upload & Management UI `/hunian-gallery` + nav "Galeri Hunian" (M19C: selector item katalog public-safe, dropzone drag-and-drop + click-to-select + preview lokal + validasi client JPEG/PNG/WebP maks 3 MB maks 10/item, upload via `/files` purpose `hunian_gallery` lalu attach ke `catalogSlug`/`publicGroupKey`, cover/publish/reorder/edit/delete + safety notice, property_owner view-only; **M19C-QA PASS dengan limitasi browser visual dan live API smoke**), Public Gallery Integration (M19D: cover image kartu `/kamar`, hero gallery + thumbnail selector + lightbox `/kamar/$slug` dengan keyboard navigation, placeholder/fallback aman, tanpa foto dummy, CTA lead/WhatsApp tidak berubah; **M19D-QA PASS dengan limitasi browser visual dan live API smoke**), final release/handoff (M19E, `docs/19-hunian-gallery/M19_HUNIAN_GALLERY_FINAL_RELEASE_HANDOFF.md`). Catatan deployment: restart API agar route M18/M19 aktif; pastikan migration 015 diterapkan; pastikan permission direktori upload storage; pastikan `VITE_API_BASE_URL` benar untuk URL media publik; redeploy app Admin dan Penghuni/public. **M19 Gallery MVP READY untuk internal/demo/staging dengan limitasi tercatat; production NOT READY**; publik hanya melihat gambar `publicVisible`; tanpa public upload; `thumbnailUrl` masih `null` (thumbnail publik memuat gambar asli); review foto (tanpa plat nomor kamar/wajah penghuni/dokumen/informasi internal) tetap SOP manual admin.
- M20A selesai (2026-07-08): Staging Restart & Visual QA Pass - plan/checklist (`docs/20-staging-visual-qa/M20_STAGING_RESTART_VISUAL_QA_PLAN.md`); milestone penutup siklus revisi, TANPA fitur baru.
- M20B selesai (2026-07-08): VPS Deployment Smoke Validation (`docs/20-staging-visual-qa/M20_STAGING_RESTART_VISUAL_QA_RESULT.md`) - API/Admin/Penghuni build PASS, API lint PASS, Admin/Penghuni typecheck PASS, migration 015 apply+replay PASS, fresh API port 3001 smoke PASS untuk M18/M19 routes dan regresi kunci, storage upload writable PASS, safety/privacy scan PASS. Browser visual QA tidak dijalankan karena tooling browser tidak tersedia di VPS dan install browser dilarang pada pass ini; media content smoke skipped karena belum ada published gallery sample. Verdict: **M20 PASS with browser visual limitation**. **M20 TIDAK mengklaim production readiness.**
