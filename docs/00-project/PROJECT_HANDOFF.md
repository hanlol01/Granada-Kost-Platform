...
- M20A selesai (2026-07-08): Staging Restart & Visual QA Pass - plan/checklist (`docs/20-staging-visual-qa/M20_STAGING_RESTART_VISUAL_QA_PLAN.md`); **eksekusi pending** (operator/Codex). Milestone penutup siklus revisi hari ini, TANPA fitur baru: checklist restart/deploy staging (pull master terbaru, migration 015, restart API agar route M18/M19 aktif + menutup limitasi port 3000 stale, redeploy Admin + Penghuni, cek `VITE_API_BASE_URL`/`VITE_PUBLIC_WHATSAPP_NUMBER`, storage upload writable, media endpoint), API smoke, admin gallery smoke, public catalog/gallery smoke, optional local browser visual QA (VPS tanpa browser tooling), safety/privacy checklist, acceptance criteria + wording verdict ("M20 PASS with browser visual limitation" / "M20 PASS with browser visual QA completed"). **M20 TIDAK mengklaim production readiness.**
