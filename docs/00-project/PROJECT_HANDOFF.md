...
> Diperbarui: 2026-07-03 (M12G). Dokumen serah terima kondisi proyek untuk engineer/agen berikutnya.
> Versi sebelumnya menggambarkan tahap migrasi monorepo awal (backend belum discaffold) dan sudah tidak berlaku.
...
- Frontend Phase 1 selesai (M11A-M11G). QA-01 Final Regression PASS (2026-07-02). Internal Demo Ready.
- M12C1-M12D selesai: File API backend-mediated, generic upload engine, upload bukti pembayaran manual Penghuni, lampiran komplain (backend readiness + UI create Penghuni), preview file Admin.
- M12E/M12F selesai: penyegaran dokumentasi & project state.
- M12G selesai: QA-M12G Cross-Scope File Security Boundary Verification dijalankan eksternal via Codex GPT-5.5 High - verdict PASS (detail: `INTERNAL_DEMO_CHECKLIST.md` Section 12).
...
Rujukan utama: `PROJECT_MASTER.md`, `ROADMAP.md`, `docs/README.md` (indeks), `DEVELOPMENT_WORKFLOW.md` (pembagian peran agen dan validasi).
