...
- **M20A Staging Restart & Visual QA Pass - plan/checklist selesai; eksekusi pending** (2026-07-08): `docs/20-staging-visual-qa/M20_STAGING_RESTART_VISUAL_QA_PLAN.md` - milestone penutup siklus revisi hari ini, TANPA fitur baru; checklist restart/deploy staging (pull master, migration 015, restart API, redeploy admin + penghuni, cek env/storage/media endpoint), API smoke, admin gallery smoke, public catalog/gallery smoke, optional local browser visual QA (VPS tanpa browser tooling), safety/privacy checklist, acceptance criteria + wording verdict ("M20 PASS with browser visual limitation" / "M20 PASS with browser visual QA completed"). Eksekusi eksternal (operator/Codex); **M20 TIDAK mengklaim production readiness**.
