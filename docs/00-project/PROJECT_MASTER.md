...
- **M19B Hunian Gallery Backend API - selesai dan tervalidasi** (2026-07-08): migration `015_hunian_gallery.sql`, purpose `hunian_gallery` di M12 File API, admin scoped gallery CRUD + set-cover + reorder + soft-delete, public media endpoint aman, dan wiring `galleryPreview/gallery` ke M18 public hunian catalog. Validasi lint/build/migration apply+replay/API smoke/regression/safety scan PASS. Tanpa frontend/admin UI, tanpa public upload, tanpa CSV/backfill, tanpa Payment Gateway/Smart Lock changes; public booking tetap NOT production-ready (`docs/19-hunian-gallery/HUNIAN_GALLERY_BACKEND_API.md`).
- **M19 (A-E) Hunian Gallery - selesai dengan limitasi tercatat** (2026-07-08): architecture/UX/safety freeze (M19A), backend gallery API tervalidasi PASS (M19B, di atas), Admin Gallery Upload & Management UI `/hunian-gallery` + nav "Galeri Hunian" (M19C; **M19C-QA PASS dengan limitasi browser visual dan live API smoke**), Public Gallery Integration cover kartu `/kamar` + hero gallery/thumbnail/lightbox `/kamar/$slug` (M19D; **M19D-QA PASS dengan limitasi browser visual dan live API smoke**), final release/handoff (M19E, `docs/19-hunian-gallery/M19_HUNIAN_GALLERY_FINAL_RELEASE_HANDOFF.md`). **M19 Gallery MVP READY untuk internal/demo/staging dengan limitasi tercatat; production NOT READY** sampai browser visual QA, deployment smoke, kebijakan storage, dan SOP review gambar selesai. Publik hanya melihat gambar `publicVisible`; tanpa public upload/video; tanpa `storage_path`/`fileId` internal/roomId/`room_code`/nomor kamar eksak/PII publik; `thumbnailUrl` masih `null` (pipeline thumbnail deferred ke M20); review foto tetap SOP manual admin; public booking tetap NOT production-ready.
- **M20 Staging Restart & Visual QA Pass - selesai dengan limitasi browser visual** (2026-07-08): plan/checklist M20A (`docs/20-staging-visual-qa/M20_STAGING_RESTART_VISUAL_QA_PLAN.md`) dan execution result M20B (`docs/20-staging-visual-qa/M20_STAGING_RESTART_VISUAL_QA_RESULT.md`). API/Admin/Penghuni build PASS, API lint PASS, Admin/Penghuni typecheck PASS, migration 015 apply+replay PASS, fresh API port 3001 smoke PASS untuk M18/M19 routes dan regresi kunci, storage upload writable PASS, safety/privacy scan PASS. Browser visual QA tidak dijalankan karena tooling browser tidak tersedia di VPS dan install browser dilarang pada pass ini; media content smoke skipped karena belum ada published gallery sample. Verdict: **M20 PASS with browser visual limitation**. **M20 TIDAK mengklaim production readiness**.
...
- **M18B selesai (2026-07-08): Backend/public hunian catalog API** (`docs/18-public-hunian-catalog/PUBLIC_HUNIAN_CATALOG_API.md`; backend/public API + dokumentasi saja). Menambahkan `GET /api/v1/public/hunian-catalog` dan detail slug di atas M16 availability + M18 public-safe content constants; `bookingLeadDefaults` kompatibel M17; no migration, no frontend, no gallery upload, no payment booking, no Payment Gateway/Smart Lock changes; lint/build/API smoke/safety scan PASS. Berikutnya M18C listing refresh.
- **M19A selesai (2026-07-08): Hunian Gallery Architecture / UX / Safety Freeze** (`docs/19-hunian-gallery/HUNIAN_GALLERY_ARCHITECTURE_UX_SAFETY_FREEZE.md`; dokumentasi saja, frozen/mengikat untuk M19B+). Galeri melekat pada item katalog hunian/unit/grup - BUKAN kamar eksak; model gambar public-safe (allowlist respons publik; tanpa `storage_path`/`fileId`/roomId/`room_code`/PII); upload/manage hanya Admin/Super Admin (Owner view-only; publik hanya `publicVisible`; tanpa public upload); reuse fondasi file M12 dengan purpose `hunian_gallery` + gallery attach API; image-only JPEG/PNG/WebP maks 3 MB, maks 10 gambar/item, tanpa video/SVG; UX admin modern (drag-and-drop, cover, reorder, publish toggle; Admin `/hunian-gallery` MVP) + galeri publik ala hotel/apartemen; milestone M19B-M19E; **public booking tetap NOT production-ready**.
