...
Status: selesai dengan limitasi browser visual (2026-07-08). Dokumen: `docs/20-staging-visual-qa/M20_STAGING_RESTART_VISUAL_QA_PLAN.md` dan `docs/20-staging-visual-qa/M20_STAGING_RESTART_VISUAL_QA_RESULT.md`. M20 adalah **milestone penutup siklus revisi M18/M19** - TANPA fitur baru; memvalidasi deployment/staging/demo smoke pasca M18/M19.
...
- M20B - VPS Deployment Smoke Validation: API/Admin/Penghuni build PASS, API lint PASS, Admin/Penghuni typecheck PASS, migration 015 apply+replay PASS, fresh API port 3001 smoke PASS untuk health, public hunian catalog list/detail, rooms summary, invalid booking lead 400, unauthenticated gallery 401, public response safety scan, dan storage upload writable. Browser visual QA tidak dijalankan karena tooling browser tidak tersedia di VPS; media content smoke skipped karena belum ada published gallery sample. Verdict: **M20 PASS with browser visual limitation**. Status: selesai.
...
- **Pasca M20**: lanjutkan hanya setelah keputusan product/release berikutnya. Production tetap NOT READY; browser visual QA, published media sample smoke, storage policy hardening, dan approval production masih perlu ditutup sebelum klaim production readiness.
