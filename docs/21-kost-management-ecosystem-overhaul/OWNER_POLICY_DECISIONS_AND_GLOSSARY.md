...
| ID | Binding policy |
| --- | --- |
| `POL-BILLING-001` | Effective 1 June 2026, both Rumah Kost and Apart Kost use duration tiers: 3–5 months Rp1.900.000/month, 6–11 months Rp1.850.000/month, and 12+ months Rp1.800.000/month. The selected rate applies to every month in the contract. Tariffs remain editable at category authority; ordinary room editing must not create a room-specific tariff. |
| `POL-BILLING-002` | Standard direct onboarding supports 3–120 months. An explicit negotiated agreement supports 1–120 months; a 1–2 month term is never a standard option. Contract rent is the agreed monthly snapshot multiplied by the complete term. Standard agreements use the effective duration tier; negotiated agreements preserve both the reference tariff and the agreed tariff. |
...
| `POL-LEASE-001` | A standard lease has a minimum duration of three months. A 1–2 month lease requires an explicit negotiated agreement with reason, actor, time, reference tariff, agreed tariff, and immutable contract value; it must not alter category or room tariff authority. |
...
| `POL-PAYMENT-001` | DP is advance rent and proof of commitment. The system pre-fills a 25% recommended initial-rent credit, but an authorized admin may record a lower agreed amount. Financial activation requires verified rent credit covering at least one agreed month, capped by total contract value; a lower payment may be recorded but does not satisfy activation. Verified DP and an eligible Booking Fee reduce rent; neither is a security deposit. |
...
| `POL-PAYMENT-009` | Booking Fee is optional: either Rp0 or at least Rp1.000.000. It is an advance rent credit that reduces the remaining contract rent and contributes toward the verified rent credit required for activation. It never funds, reduces, or substitutes for the security-deposit liability. |
...
| `DEC-LEASE-001` | Direct onboarding is a full-page two-stage flow: `Penghuni & Penyewaan` and `Pilih Kamar Kost`. Room choices are vacant, gender-compatible, category-filterable, and single-select. Step one accepts a valid historical/current/future start date, a standard 3–120-month term, or an explicit 1–120-month negotiated agreement. |
| `DEC-LEASE-002` | Lead-based onboarding preloads trusted lead fields, selected category/gender, payment commitment, and Admin-selected room; incomplete identity and lease fields remain mandatory. Before Commit Onboarding, Admin may revise start date, duration, and—through the explicit negotiated path—agreed tariff. The server recalculates and freezes the commercial snapshot; recorded Booking Fee/DP/full-settlement entries remain immutable rent credits and must not exceed revised contract rent. |
...
| ID | Binding decision |
| --- | --- |
| `DEC-BILLING-001` | Lease activation creates an authoritative settlement schedule from the 1–120-month commercial snapshot. The schedule uses the agreed monthly tariff, preserves the existing 3/6/12 checkpoint outcomes, and generalizes them deterministically to every supported duration without changing contract value. |
...
| `DEC-REPORT-001` | Reports have five subpages: Penyewaan, Pembayaran, Pengeluaran, Keuangan, and Setoran Owner. `Keuangan` v1 is an explicit cash-flow report, not a full accrual profit-and-loss statement; `Setoran Owner` is a dedicated review, publication, and payout workspace rather than a generic flat report. |
...
| `DEC-OWNER-006` | Current standard economics per earned occupied-room month deduct a fixed Rp300.000 Kostation Management Fee from the lease's selected duration-tier rate. Owner Entitlement is therefore Rp1.600.000 for 3–5 months, Rp1.550.000 for 6–11 months, or Rp1.500.000 for 12+ months. Pricing tiers and management fee are separate effective-dated authorities; fee changes start on the first day of a future month and never rewrite earned historical periods. |
...
| `DEC-OWNER-009` | One monthly Owner Settlement follows `draft -> ready_for_review -> approved -> paid`; payout requires Admin approval. Publication to the Property Owner is an explicit visibility command separate from financial status. Ownership transfer attribution follows earned service coverage, not merely payment receipt date. |
| `DEC-OWNER-011` | Owner contract-payment progress is a dynamic projection of immutable contract value and eligible verified rent allocations. It may update after authoritative financial or lifecycle events but is not an invoice, final settlement, or proof of payout. |
| `DEC-OWNER-012` | Admin reviews system-calculated Owner finance and may append a typed, reasoned adjustment with evidence. Admin cannot overwrite Gross Earned Rent, Management Fee, Owner Entitlement, payout total, or payable remainder. |
| `DEC-OWNER-013` | An approved settlement remains hidden from the Property Owner until Admin explicitly publishes it. A published settlement is immutable and versioned; later correction creates linked append-only history. |
| `DEC-OWNER-014` | Owner documents are named by purpose: `Laporan Progres Hak Owner` for dynamic information, `Laporan Hak dan Setoran Owner` for the published period snapshot, and `Bukti Setoran Owner` for an actual payout. `Invoice` remains reserved for a receivable document. |
| `DEC-OWNER-015` | The Owner Portal may show full-contract collection goals and projections, while the official Owner Settlement remains monthly and recognizes only verified rent allocated to service already delivered. |
...
| `INV-OWNER-005` | Live Owner progress is derived and must not become a second financial ledger. An approved publication freezes the exact source scope, rows, totals, methodology, version, and checksum used by its PDF/XLSX documents. |
| `INV-OWNER-006` | Preparing, reviewing, approving, publishing, and paying an Owner Settlement are distinct authorities. No UI action or service command may silently combine publication or payout with calculation. |
...
| --- | --- | --- |
| `DEC-BILLING-010` | Flexible 1, 6, or arbitrary-month lease options discussed during exploration. | Superseded by `POL-LEASE-001` and `POL-BILLING-002`: standard terms are 3–120 months; 1–2 months require the explicit negotiated-agreement path, and any 3–120 month term may also use that path. |
...
| `OWNER_CONFIRMATION_REQUIRED-011` | Whether a rare 1–2 month lease can be approved and by whom. | Resolved by the latest product decision: an in-scope authorized Admin may record it only through the explicit negotiated-agreement path with complete reason and audit metadata; ordinary onboarding still starts at three months. |
...
| ID                    | Final policy                                                                                                                                                                                                                                                                                                                                                                              |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POL-SETTLEMENT-001`  | Every activated lease has one contract-rent balance and an immutable versioned checkpoint schedule. Verified Booking Fee, DP, and later rent payments all count as cumulative rent credit; security deposit never does. Existing `lease_settlement_v2` schedules remain unchanged. New `lease_settlement_v3` schedules follow the approved 1–120-month offsets and cumulative targets without creating duplicate rent principal. |
