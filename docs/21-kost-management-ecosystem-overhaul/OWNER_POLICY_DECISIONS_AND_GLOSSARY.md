...
| `DEC-OWNER-001` | `owner` means global operational owner. `property_owner` means investor/building owner with read-only access limited to assigned buildings. |
| `DEC-OWNER-002` | Every building is initially owned by KOSTATION until an active ownership assignment says otherwise. One building may have only one active investor ownership assignment at a time. |
| `DEC-OWNER-003` | Investor provisioning creates/reuses an account, assigns building scope, delivers a temporary credential once, and requires first-login password change. |
| `DEC-OWNER-004` | Property Owner may read dashboard, rooms, resident/lease summaries, payments, vehicles, booking interest, complaints, notifications, and reports only for owned buildings. Sensitive identity documents, full NIK, credentials, and unrelated property data are excluded by default. |
...
| `INV-OWNER-001` | Property Owner reads only records whose authoritative building relation intersects an active ownership assignment at the record's effective time. |
...
| **Property Owner** | Investor/building owner with read-only building scope. Technical role: `property_owner`. | `owner`, `cabang owner` |
...
| `DEC-OWNER-010` | Role name `owner` was used for building investor. | Global operator remains `owner`; investor is `property_owner` with building scope. |
...
| `OWNER_CONFIRMATION_REQUIRED-007` | Investor purchase price, ownership document workflow, revenue share, effective-date approval, and transfer settlement. | Implement building assignment/read-only scope only; keep commercial acquisition workflow disabled. |
