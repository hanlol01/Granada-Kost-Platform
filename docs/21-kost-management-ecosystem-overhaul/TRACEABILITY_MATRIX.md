...
| `KMO-W10-OWNER-A3 / C`                      | Additive 037 allocation-backed exact half-open earning service coverage; active rent allocation/invoice/lease/occupancy reconciliation; coverage-bound settlement/adjustment/payout and financial notification lineage; clipped historical projections; embedded-Unicode `property_owner` read-only portal/report/export | Focused W10 authority/portal contracts, real Type0/CID/ToUnicode PDF text extraction plus XLSX detail extraction, Admin strict parser/navigation/state contracts, API/Admin build; disposable PostgreSQL first-apply/replay/allocation/lifecycle/constraint/rollback/cleanup proof is executable when `KOSTATION_POSTGRES_BIN` is available; canonical migration and browser/runtime remain deferred | SOURCE_IMPLEMENTED; AUTOMATED REVIEW PENDING; RUNTIME DEFERRED |

Aggregate status: `KMO-W00–W06 — AUTOMATED VERIFIED; RUNTIME DEFERRED. KMO-W10-OWNER-C — SOURCE_IMPLEMENTED; AUTOMATED REVIEW PENDING; RUNTIME DEFERRED`.
...
runtime/browser proof remains deferred. W10-OWNER-C has focused source evidence
only; its export parity, reconciliation, and authenticated browser evidence remain
deferred. W07 onward is still planned and is not advanced by these delivery rows.
